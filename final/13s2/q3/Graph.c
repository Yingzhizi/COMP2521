...
int checkAllAssigned(int *ComponentOf, int size);
void DFS(Graph g, Vertex v, int *ComponentOf);
...
int nComponents(Graph g)
{
	...
	while (checkAllAssigned(ComponentOf, g->nV) == 0) {
		Vertex v;
		for (v = 0; v < g->nV; v++) {
			if (ComponentOf[v] == -1) {
				break;
...
void DFS(Graph g, Vertex v, int *ComponentOf) {
	...
	for (w = 0; w < g->nV; w++) {
		if (g->edges[v][w] == 1 && g->edges[w][v] == 1 && ComponentOf[w] == -1) {
			DFS(g, w, ComponentOf);
		}
	}
}
