...
{
   // TODO
   // initialise the visited path to -1
   for (int i = 0; i < g->nV; i++) {
       visited[i] = -1;
   }
   // check all the vertex v except from itself, see if there has path
   // from s to v??? using bfs by queue
   // create a new queue
   Queue new = newQueue();
   // push the souce item to queue
   QueueJoin(new, s);

   // mark the souce item been visited
   visited[s] = 0;
   int index = 0;
   // start BFS
   while (!QueueIsEmpty(new)) {
       // dequeue the first item in the queue
       Vertex curr = QueueLeave(new);

       if (curr != s && visited[curr] <= d) {
           vs[index++] = curr;
       }
       // if the current value is not the same as the source
           // check the neighbour
       Vertex v;
       for(v = 0; v < g->nV; v++) {
           if (g->edges[curr][v] && visited[v] == -1) {
