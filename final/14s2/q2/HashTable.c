...
{
    int newLength = 2 * ht->nslots;
    List *new = malloc(newLength * sizeof(List));
    assert(new != NULL);
    for (int i = 0; i < newLength; i++) {
        new[i] = newList();
    }
    // now we need to append the old kay value into the new hashTable
    for (int i = 0; i < ht->nslots; i++) {
		int n;
		int *values = valuesFromList(ht->chains[i], &n);
		for (int j = 0; j < n; j++) {
			int h = hash(values[j],newLength);
			appendList(new[h],values[j]);
		}
		free(values);
	}

    // clean the memory of the old chains
    for (int i = 0; i < ht->nslots; i++) {
        dropList(ht->chains[i]);
    }
    free(ht->chains);
