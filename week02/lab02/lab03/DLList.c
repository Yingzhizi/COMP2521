...
void DLListBefore(DLList L, char *it)
{
	...
	//if the node is inserted before the first node
	//it becomes the new first node
...
void DLListDelete(DLList L)
{
	...
	assert (L != NULL);
	//the list is empty, do not need to do anything
	DLListNode *temp;
	if (L->first == NULL) {
	    return;
	}
	//one node
	else if (DLListLength(L) == 1) {
		free(L->curr->value);
		free(L->curr);
	...
	//if delete the first node
		...
		DLListNode *store_prev = L->curr->prev;
		DLListNode *store_next = L->curr->next;
