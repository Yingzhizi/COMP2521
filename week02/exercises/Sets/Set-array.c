...
//Same as
	/*if (s == NULL) {
		return 0;
	} else if (s->nelems < 0 || s->nelems > MAXELEMS) {
		return 0;
	} else {
		return 1;
	}
...
void SetInsert(Set s, int n)
{
	// assert(isValid(s));
	int i;
	//set cannot exist same value, if find the inserted value has already exist int the set
	//return
	for (i = 0; i < s->nelems; i++)
		if (s->elems[i] == n) return;

	//insert value n to the end of the set
	s->elems[s->nelems] = n;
	s->nelems++;
}
